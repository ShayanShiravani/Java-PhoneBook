...
public class ContactPageController {
	...
	private void handleDeleteContact() {
		...
		if (selectedIndex >= 0) {
			...
			showContactDetails(allContacts.getSelectionModel().getSelectedItem());
			
		} else {
			Dialogs.create()
    		.title("No contact selected")
    		.masthead("No contact selected")
	...
	private void handleEditContact() {
		...
		if (selectedContact != null) {
			...
			if (saveClicked) {
				allContacts.getColumns().get(0).setVisible(false);
				allContacts.getColumns().get(0).setVisible(true);
				allContacts.getSelectionModel().select(selectedContact);
